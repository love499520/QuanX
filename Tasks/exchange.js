...
$.http.get({url: "https://raw.githubusercontent.com/fawazahmed0/currency-api/1/latest/currencies/cny.json"})
    .then((response) => {
        ...
        const info = Object.keys(currencyNames).reduce((accumulator, key) => {
            let line = "";
            if (key !== base && data.cny.hasOwnProperty(key)) {
                const rate = parseFloat(data.cny[key]);
